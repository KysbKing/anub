while True:
    try:
        ...
        if platform.system() == "Windows":
            ...
            enter = pystyle.Colorate.Horizontal(pystyle.Colors.white_to_blue, ('Welcome to Anubis, Press "ENTER" to continue!'))
            pystyle.Anime.Fade(
       ...
       ▀███████████ ███   ███ ███    ███ ▀▀███▀▀▀██▄  ███▌ ▀███████████  
         ...
         ███    █▀   ▀█   █▀  ████████▀  ▄█████████▀  █▀    ▄████████▀
                ...
                Welcome to Anubis, Press "ENTER" to continue! Grestal'''), pystyle.Colors.white_to_blue, pystyle.Colorate.Vertical, enter=True)
        def Main():
            if platform.system() == "Windows":
                os.system("cls")
...
▀███████████ ███   ███ ███    ███ ▀▀███▀▀▀██▄  ███▌ ▀███████████  
  ...
  ███    █▀   ▀█   █▀  ████████▀  ▄█████████▀  █▀    ▄████████▀    
                                                                      ...
                                                                      \n'''), pystyle.Colors.white_to_blue, interval = 0.0005)
            else:
                os.system("clear")
                pystyle.Write.Print(pystyle.Center.XCenter('''                                                                   
                                    _     _     
...
'''), pystyle.Colors.white_to_blue, interval = 0.0005)
        Main()
        while True:
            choice = pystyle.Write.Input("\n\n[?] Выберите пункт меню -> ", pystyle.Colors.white_to_blue, interval = 0.001)
            if choice == "1":
                phone = pystyle.Write.Input("\n[?] Введите номер телефона -> ", pystyle.Colors.white_to_blue, interval = 0.005)
                def phoneinfo(phone):
                    try:
                        parsed_phone = phonenumbers.parse(phone, None)
                        if not phonenumbers.is_valid_number(parsed_phone):
                            return pystyle.Write.Print(f"\n[!] Произошла ошибка -> Недействительный номер телефона\n", pystyle.Colors.white_to_blue, interval=0.005)
...
[+] Viber -> https://viber.click/{phone}\n"""
                        pystyle.Write.Print(print_phone_info, pystyle.Colors.white_to_blue, interval=0.005)
                    except Exception as e:
                        pystyle.Write.Print(f"\n[!] Произошла ошибка -> {str(e)}\n", pystyle.Colors.white_to_blue, interval=0.005)
                phoneinfo(phone)
            if choice == "2":
                domain = pystyle.Write.Input("\n[?] Введите сайт -> ", pystyle.Colors.white_to_blue, interval = 0.005)
...
[+] IP-адрес: {domain_info.name_servers}
        """
                    pystyle.Write.Print(print_string, pystyle.Colors.white_to_blue, interval=0.005)
                get_website_info(domain)
            if choice == "3":
                nick = pystyle.Write.Input(f"\n[?] Введите никнейм -> ", pystyle.Colors.white_to_blue, interval=0.005)
                urls = [
                ...
                for url in urls:
                    try:
                        ...
                        if response.status_code == 200:
                            pystyle.Write.Print(f"\n{url} - аккаунт найден", pystyle.Colors.white_to_blue, interval=0.005)
                        elif response.status_code == 404:
                            pystyle.Write.Print(f"\n{url} - аккаунт не найден", pystyle.Colors.white_to_blue, interval=0.005)
                        else:
                            pystyle.Write.Print(f"\n{url} - ошибка {response.status_code}", pystyle.Colors.white_to_blue, interval=0.005)
                    except:
                        pystyle.Write.Print(f"\n{url} - ошибка при проверке", pystyle.Colors.white_to_blue, interval=0.005)
                print()
            if choice == "4":
                ip = pystyle.Write.Input("\n[?] Введите IP-адрес -> ", pystyle.Colors.white_to_blue, interval = 0.005)
                def ip_lookup(ip):
                ...
                print()
                pystyle.Write.Print(ip_lookup(ip), pystyle.Colors.white_to_blue, interval=0.005)
            if choice == "5":
                path = pystyle.Write.Input("\n[?] Введите путь к БД: ", pystyle.Colors.white_to_blue, interval=0.005)
                search_text = pystyle.Write.Input("\n[?] Введите текст:  ", pystyle.Colors.white_to_blue, interval=0.005)
                print()
                try:
                    with open(path, "r", encoding="utf-8") as f:
                        for line in f:
                            if search_text in line:
                                pystyle.Write.Print("[+] Результат: " + line.strip(), pystyle.Colors.white_to_blue, interval=0.005)
                                print()
                                break
                        else:
            ...
            if choice == "6":
                url = pystyle.Write.Input("[?] URL: ", pystyle.Colors.white_to_blue, interval=0.005)
                num_requests = int(
                    pystyle.Write.Input(
                        "[?] Введите количество запросов: ", pystyle.Colors.white_to_blue, interval=0.005
                    )
                )
                user_agents = [
                ...
                def send_request(i):
                    ...
                    try:
                        ...
                        print(f"{colorama.Fore.white_to_blue}[+] Request {i} sent successfully\n")
                    except:
                        print(f"{colorama.Fore.white_to_blue}[+] Request {i} sent successfully\n")
                threads = []
            ...
            if choice == "7":
                def get_proxy():
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            pystyle.Write.Print(f"\nПроизошла ошибка -> {response.status_code}", pystyle.Colors.white_to_blue, interval=0.005)
                    except Exception as e:
                        pystyle.Write.Print(f"\nПроизошла ошибка -> {str(e)}", pystyle.Colors.white_to_blue, interval=0.005)
                ...
                if proxies:
                    ...
                    for proxy in proxies:
                        pystyle.Write.Print("\n" + proxy, pystyle.Colors.white_to_blue, interval=0.005)
                    print()
                else:
                    pystyle.Write.Print("Прокси недоступно.", pystyle.Colors.white_to_blue, interval=0.005)
            if choice == "8":
            ...
            if choice == "9":
                ...
                def generate_password(length, complexity):
                    ...
                    return password
                password_length = int(
                    pystyle.Write.Input("[?] Введите длину пароля -> ", pystyle.Colors.white_to_blue, interval=0.005)
                )
                complexity = pystyle.Write.Input(
                    "[?] Выберите сложность (low, medium, high): ", pystyle.Colors.white_to_blue, interval=0.005)
                print()
...
Отправили письмо - следуйте этим же шагам в правильной последовательности. Выйдите из Tor, Linux, а затем из VPN.\n''', pystyle.Colors.white_to_blue, interval = 0.005)
            if choice == '19':
                pystyle.Write.Print("\n[?] Выберите режим: ", pystyle.Colors.white_to_blue, interval=0.005)
                pystyle.Write.Print("\n\n[?] 1 - проверить часто используемые порты", pystyle.Colors.white_to_blue, interval=0.005)
                pystyle.Write.Print("\n\n[?] 2 - проверить указанный порт", pystyle.Colors.white_to_blue, interval=0.005)
                mode = pystyle.Write.Input("\n\n[?] Ваш выбор: ", pystyle.Colors.white_to_blue, interval=0.005)
                if mode == "1":
                    print()
                    ports = [
                        20,
                    ...
                    for port in ports:
                        ...
                        if result == 0:
                            pystyle.Write.Print(f"[+] Порт {port} открыт", pystyle.Colors.white_to_blue, interval=0.005)
                        else:
                            pystyle.Write.Print(f"[+] Порт {port} закрыт", pystyle.Colors.white_to_blue, interval=0.005)
                        sock.close()
                        print()
                elif mode == "2":
                    port = pystyle.Write.Input("\n[?] Введите номер порта: ", pystyle.Colors.white_to_blue, interval=0.005)
                    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    ...
                    pystyle.Write.Print("\n[!] Неизвестный режим", pystyle.Colors.white_to_blue, interval=0.005)
                    print()
            if choice == "14":
                fake = faker.Faker(locale="ru_RU")
                gender = pystyle.Write.Input("\n[?] Введите пол (М - мужской, Ж - женский): ", pystyle.Colors.white_to_blue, interval=0.005)
                print()
            ...
            if choice == "15":
                start_url = pystyle.Write.Input("[?] Введите ссылку -> ", pystyle.Colors.white_to_blue, interval=0.005)
                max_depth = 2
                visited = set()
                ...
                def dos1():
                    try:
                        ...
                        def make_request(url):
                            ...
                            response = requests.get(url, headers=headers)
                            print(f"\n{colorama.Fore.white_to_blue}[{colorama.Fore.WHITE}+{colorama.Fore.white_to_blue}]{colorama.Fore.MAGENTA} Атака началась, состояние сайта: ", response.status_code)
                        def dos():
                            url = input(f"{colorama.Fore.white_to_blue}[{colorama.Fore.WHITE}?{colorama.Fore.white_to_blue}]{colorama.Fore.MAGENTA} Введите ссылку : ")
                            power = input(f"{colorama.Fore.white_to_blue}[{colorama.Fore.WHITE}?{colorama.Fore.white_to_blue}]{colorama.Fore.MAGENTA} Выберите режим (1 - Слабый/2 - Средний/3 - Мощный) : ")
                            if power == "1":
                                ...
                                print(f"\n{colorama.Fore.white_to_blue}[{colorama.Fore.WHITE}!{colorama.Fore.white_to_blue}]{colorama.Fore.MAGENTA} Нет такого режима!")
                        dos()
                    except:
                        print(f'\n{colorama.Fore.white_to_blue}[{colorama.Fore.WHITE}!{colorama.Fore.white_to_blue}]{colorama.Fore.MAGENTA} Произошла ошибка! Проверьте ввод данных!')
                dos1()
            if choice == "17":
                pystyle.Write.Print("\n[?] Выберите страну:\n", pystyle.Colors.white_to_blue, interval=0.005)
                ...
                pystyle.Write.Print("\n[+] Страна: " + card["Страна"], pystyle.Colors.white_to_blue, interval=0.005)
                pystyle.Write.Print("\n[+] Номер карты: " + card["Номер карты"], pystyle.Colors.white_to_blue, interval=0.005)
                pystyle.Write.Print("\n[+] Срок действия: " + card["Срок действия"], pystyle.Colors.white_to_blue, interval=0.005)
                pystyle.Write.Print("\n[+] CVV: " + card["CVV"] + "\n", pystyle.Colors.white_to_blue, interval=0.005)
            if choice == "18":
                ...
                mac_address = pystyle.Write.Input("[?] Введите Mac-Address -> ", pystyle.Colors.white_to_blue, interval = 0.005)  # Replace this with the MAC address you want to lookup
        ...
        "Mozilla/5.0 (X11; Linux x86_64; rv:{0}.0) Gecko/{0}{1:02d} Firefox/{0}.0",
                    ...
                    for _ in range(num_agents):
                        ...
                        user_agent = random.choice(versions).format(version, year, year, month)
                        pystyle.Write.Print("\n" + user_agent, pystyle.Colors.white_to_blue, interval = 0.005)
                generate_user_agents(int(num_agents))
                print()
            if choice == "21":
                pystyle.Write.Print("\n[+] ДОНАТ -> нету, создатель Grestal", pystyle.Colors.white_to_blue, interval = 0.005)
            if choice == "22":
                pystyle.Write.Print("\n[+] СОЗДАТЕЛЬ -> Ориг создатель скрипта Grestal\n", pystyle.Colors.white_to_blue, interval = 0.005)
            if choice == "23":
                sure = pystyle.Write.Input("Вы действительно хотите выйти? Y/N -> ", pystyle.Colors.white_to_blue, interval = 0.005)
                if sure.lower() == "y" or sure.lower() == "yes" or sure.lower() == "н" or sure.lower() == "нуы" or sure.lower() == "да" or sure.lower() == "lf":
                    sys.exit()
                else:
                    continue
            pystyle.Write.Input("\n[?] Нажмите Enter для продолжения", pystyle.Colors.white_to_blue, interval = 0.005)
            Main()
    except Exception as e:
